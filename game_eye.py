...
def main_game():
    ...
    while (running and webcam.isOpened()):
        ...
        if (result.multi_face_landmarks):
            ...
            left_iris_pts = [eyes_detection.to_px(landmarks[i], frameW, frameH) for i in LEFT_IRIS_IDX]
            right_iris_pts = [eyes_detection.to_px(landmarks[i], frameW, frameH) for i in RIGHT_IRIS_IDX]
            
            # Find the mean coordinates within the range of coordinates
            left_iris_center  = eyes_detection.avg_points(left_iris_pts)
            right_iris_center = eyes_detection.avg_points(right_iris_pts)

            # Smooth each eye separately, then average for a robust "pupil" center
            ema_left  = eyes_detection.ema_update(ema_left,  left_iris_center, SMOOTH_ALPHA)
            ema_right = eyes_detection.ema_update(ema_right, right_iris_center, SMOOTH_ALPHA)

            # Use the midpoint of the two smoothed irises as the single "gaze point"
            iris_center = eyes_detection.avg_points([ema_left, ema_right])
        ...
        # Get command
        command = eyes_detection.determine_command(iris_center, screen_center, DEAD_ZONE_HORIZONTAL, DEAD_ZONE_VERTICAL) 
